...
(window["webpackJsonp"] = window["webpackJsonp"] || []).push([["roles-roles-module"], {
  ...
  function srcAppLayoutSetupConfigurationRolesCreateRoleCreateRoleComponentTs(module, __webpack_exports__, __webpack_require__) {
    ...
    var _permissions_json__WEBPACK_IMPORTED_MODULE_4___namespace =
    /*#__PURE__*/
      ...
      _createClass(CreateRoleComponent, [{
        ...
        value: function btnSubmitHandler() {
          ...
          this.roleService.addRole(data).subscribe(function (r) {
            if (r.status) {
              _this.toasterMessage('success', r.message);

              _this.rolePermissionForm.reset();

              _this.router.navigateByUrl('setup-configuration/roles');
            } else {
              _this.toasterMessage('error', r.message);
            }
          });
        }
      }, {
      ...
      _createClass(EditRoleComponent, [{
        ...
        value: function ngOnInit() {
          ...
          this.roleService.getRole(this.selectedRoleId).subscribe(function (r) {
            if (r.status) {
              _this2.selectedRole = r.data;

              _this2.createRolePermissionForm(r.data.code, r.data.name, r.data.permissions);
            } else {
              _this2.toasterMessage('error', 'Role Name & code is required.');
            }
          });
        }
      }, {
        ...
        value: function btnSubmitHandler() {
          ...
          this.roleService.updateRole(this.selectedRoleId, data).subscribe(function (r) {
            if (r.status) {
              _this3.toasterMessage('success', r.msg);

              _this3.rolePermissionForm.reset();

              _this3.router.navigateByUrl('setup-configuration/roles');
            } else {
              _this3.toasterMessage('error', r.message);
            }
          });
        }
      }, {
            ...
            for (var _iterator3 = this.allPermissions[Symbol.iterator](), _step3; !(_iteratorNormalCompletion3 = (_step3 = _iterator3.next()).done); _iteratorNormalCompletion3 = true) {
              ...
              try {
                var _loop = function _loop() {
                  ...
                  _this4.rolePermissionForm.addControl(dsKey, new _angular_forms__WEBPACK_IMPORTED_MODULE_2__["FormControl"](isAllowed[dsKey]));
                };
      ...
      _createClass(RolesComponent, [{
        ...
        value: function fetchRoles() {
          ...
          this.roleService.getRoles().subscribe(function (res) {
            ...
            _this5.toasterMessage('error', err);
          });
        }
      }, {
        key: "delete",
        value: function _delete(role) {
          ...
          sweetalert2__WEBPACK_IMPORTED_MODULE_2___default.a.fire({
            ...
            if (result.value) {
              _this6.roleService.deleteRole(role).subscribe(function (r) {
                if (r.status) {
                  _this6.roles.filter(function (value, index, arr) {
                    if (role === value['role_id']) {
                      arr.splice(index, 1);
                    }
                  });

                  _this6.toasterMessage('success', 'Role deleted successfully');
                } else {
                  _this6.toasterMessage('error', r.msg);
                }
              }, function (err) {
                _this6.toasterMessage('error', err);
              });
            }
          });
        }
